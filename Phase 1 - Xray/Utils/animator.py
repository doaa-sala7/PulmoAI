...
def animate_plot(
    ...
    if title == None:
        fig.update_layout(
            title=Plot_mode + " history of model",
            xaxis_title="Epochs",
...
def learning_rate_plot(history, Plot_mode="lr"):
    ...
    fig.update_layout(
        yaxis=dict(tickmode="linear", tick0=0, dtick=max(history[Plot_mode] / 5))
    )
